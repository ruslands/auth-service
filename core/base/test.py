...
class TestBase:
    ...
    async def test_create(self, test_client):
        ...
        pytest.test_entity_id = response.json()["data"]["id"]
    ...
    async def test_get(self, test_client):
        response = test_client.get(
            f"{self.url}/{pytest.test_entity_id}",
            headers={"Authorization": f"Bearer {pytest.test_token}"}
        )
        assert response.status_code == 200
    ...
    async def test_delete(self, test_client):
        response = test_client.delete(
            f"{self.url}/{pytest.test_entity_id}", headers={"Authorization": f"Bearer {pytest.test_token}"})
