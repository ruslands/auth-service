...
import requests
from integration.aws import get_lambda_client
from sqlalchemy.ext.asyncio import AsyncSession

from app.rbac.schema import IRBACValidate
from app.rbac.service import RBAC
...
def user_validate(access_token, email, communication):
    ...
    if communication == "api":
        ...
        response = requests.get(
            url='%s/api/auth/v1/user/list?page=1&size=1&filters={"email": "%s"}' % (os.getenv("APP_BASE_URL"), email),
            headers={"Authorization": access_token},
        )
        logger.debug(f"/user/list response_status: {response.status_code}; response: {response.text}")
        ...
        if not os.environ["APP_FUNCTION_ARN"]:
            raise ForbiddenException(detail="APP_FUNCTION_ARN not set, will not continue")
        ...
        logger.debug(f"/user/list response: {response}")
...
async def rbac_validate(request, communication, db_session: Optional[AsyncSession] = None):
    ...
    if communication == "api":
        ...
        logger.debug(f"/rbac/validate response_status: {response.status_code}; response: {response.text}")
        if response.status_code == 200:
            if not response.json()["data"]["is_access_allowed"]:
        ...
        rbac = RBAC()
        response = await rbac.validate(db_session=db_session, req=IRBACValidate(**data), access_token=access_token)

        if not response["is_access_allowed"]:
            raise ForbiddenException
        ...
        if lambda_status_code != 200:
            raise ForbiddenException
        response = json.loads(response["Payload"].read())
        logger.debug(f"/rbac/validate response: {response}")
        status_code = response["statusCode"]
        response = json.loads(response["body"])
        ...
        if status_code == 200:
            if not response["data"]["is_access_allowed"]:
                raise ForbiddenException
...
def visibility_group_validate(access_token, visibility_group_entity, communication):
    ...
    if communication == "api":
        ...
        logger.debug(f"/visibility_group/validate response_status: {response.status_code}; response: {response.text}")
        if response.status_code == 200:
        ...
        if lambda_status_code != 200:
            raise ForbiddenException
        response = json.loads(response["Payload"].read())
        logger.debug(f"/visibility_group/validate response: {response}")
        status_code = response["statusCode"]
        response = json.loads(response["body"])
        if isinstance(response, dict) and "error" in response.keys():
            raise ForbiddenException
