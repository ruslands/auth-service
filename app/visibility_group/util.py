...
class VisibilityGroup:
    ...
    async def get(
        ...
        if (int(datetime.now().timestamp()) - self.visibility_update_timestamp) > self.VISIBILITY_UPDATE_DELAY:
            await self.update(db_session)
            self.visibility_update_timestamp = int(datetime.now().timestamp())
    ...
    async def update(
        ...
        self.visibility = await crud.visibility_group.get_visibility_group_and_users(db_session)
        self.visibility = {i.prefix: IVisibilityGroupSettings.parse_obj(i)
                           for i in self.visibility}
    ...
    async def validate(
        ...
        if payload['visibility_group'] not in visibility_groups:
            raise ConflictException(detail="Visibility group user belongs to does not exist")
        ...
        if 'admin' in visibility_group[visibility_group_entity] and str(visibility_group['admin']) == str(payload['user_id']):
            ...
            response['users'].extend([{"id": UUID(payload['user_id']), "email": payload['email']}])

        # looking for child visibility groups
        for prefix, value in visibility_groups.items():
            if prefix.startswith(visibility_group['prefix']) and prefix != visibility_group['prefix']:
                value = value.dict()
