...
__all__ = ("get_secret", "YCAuthMethod")
...
def get_secret(secret_id: str, auth_type: str, credentials: str) -> dict:
    """
    Authorization method is determined by the auth_type environment variable.
    ...
    try:
        match auth_type:
            case YCAuthMethod.YC_AUTH_BY_SERVICE_ACCOUNT_KEY:
                ...
                yc_sdk = yandexcloud.SDK(service_account_key=credentials)
            case YCAuthMethod.YC_AUTH_BY_OAUTH_TOKEN:
                yc_sdk = yandexcloud.SDK(token=credentials)
            case YCAuthMethod.YC_AUTH_BY_IAM_TOKEN:
                # yc_sdk = yandexcloud.SDK(iam_token=credentials)
        ...
        return {entry.key: entry.text_value for entry in response.entries}
